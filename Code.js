...
const API_KEY = "AIzaSyBr7zgaTnsxmYP8hcjYs_SlYQWmhVZd6zo"; 
const FOLDER_ID = "1xBKm0U79cnWQj3oTsNVz63HH65fgv-my"; 
const SCRIPT_VERSION = "27/01 17:00"; 

// Credentials Εταιρείας
const ADMIN_USER = "support@datalink.com.gr";
const ADMIN_PASS = "Datalink123!@#";
...
function getBulkMailTaskQueue(messageIds) {
  const projectId = 'gen-lang-client-0465952145';
  const datasetId = 'summy_logs';
  ...
  try {
    const threadId = GmailApp.getMessageById(messageIds[0]).getThread().getId();
    ...
    // Αρχικοποιούμε όλα τα messageIds με άδειο array
    messageIds.forEach(id => { results[id] = []; });
    
    if (res.rows) {
      res.rows.forEach(r => {
        const mId = r.f[0].v; // Το message_id από τη BQ
        if (results[mId]) {
          results[mId].push({
            task: r.f[1].v,
